...
def _command_for(script: Path, args: argparse.Namespace) -> list[str]:
    # PATH can expose PyGitIndex through an executable wrapper. Only actual
    # Python sources should be forced through this interpreter.
    command = [sys.executable, str(script)] if script.suffix.casefold() in {".py", ".pyw"} else [str(script)]
