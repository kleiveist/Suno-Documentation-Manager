...
EXPECTED_INPUTS = {
    "sunodm-desktop-linux-unsigned",
    "sunodm-desktop-macos-unsigned",
    "sunodm-desktop-windows-unsigned",
    "sunodm-web-release-validation",
}
DESKTOP_PREARCHIVES = {
    "linux": "sunodm-desktop-linux-unsigned.tar.gz",
    "macos": "sunodm-desktop-macos-unsigned.tar.gz",
    "windows": "sunodm-desktop-windows-unsigned.zip",
}
CHECKSUM_PATTERN = re.compile(r"([0-9a-f]{64})  ([A-Za-z0-9][A-Za-z0-9._-]*)")
POLYFORM_SHIELD_MARKERS = (
...
WEB_DISTRIBUTION_FILES = ("LICENSE", "NOTICE", "THIRD_PARTY_NOTICES.md")
...
def _create_source_archive(root: Path, identity: ReleaseIdentity, destination: Path) -> None:
    ...
    _validate_source_licensing(destination, identity)


def _validate_source_licensing(path: Path, identity: ReleaseIdentity) -> None:
    prefix = f"sunodm-{identity.tag}/"
    try:
        with zipfile.ZipFile(path) as archive:
            license_text = archive.read(f"{prefix}LICENSE").decode("utf-8")
            notice_text = archive.read(f"{prefix}NOTICE").decode("utf-8")
            third_party_notices = archive.read(f"{prefix}THIRD_PARTY_NOTICES.md").decode("utf-8")
    except (KeyError, UnicodeDecodeError, zipfile.BadZipFile) as exc:
        raise ReleasePublishError(
            "source archive is missing a valid UTF-8 LICENSE, NOTICE, or THIRD_PARTY_NOTICES.md"
        ) from exc
    if any(marker not in license_text for marker in POLYFORM_SHIELD_MARKERS):
        raise ReleasePublishError("source archive does not contain the PolyForm Shield 1.0.0 license")
    if tuple(notice_text.splitlines()) != REQUIRED_NOTICE_LINES:
        raise ReleasePublishError("source archive does not contain the exact required licensor NOTICE")
    if not third_party_notices.strip():
        raise ReleasePublishError("source archive has an empty THIRD_PARTY_NOTICES.md")


def _validate_web_distribution(path: Path, source_archive: Path, identity: ReleaseIdentity) -> None:
    try:
        with zipfile.ZipFile(path) as archive:
            _validate_archive_names(archive.namelist())
            invalid = archive.testzip()
            if invalid is not None:
                raise ReleasePublishError(f"web archive has a corrupt member: {invalid}")
            packaged = {name: archive.read(name) for name in WEB_DISTRIBUTION_FILES}
        prefix = f"sunodm-{identity.tag}/"
        with zipfile.ZipFile(source_archive) as archive:
            expected = {name: archive.read(f"{prefix}{name}") for name in WEB_DISTRIBUTION_FILES}
    except (KeyError, OSError, zipfile.BadZipFile) as exc:
        raise ReleasePublishError("web archive is missing a valid LICENSE, NOTICE, or THIRD_PARTY_NOTICES.md") from exc
    mismatches = [name for name in WEB_DISTRIBUTION_FILES if packaged[name] != expected[name]]
    if mismatches:
        raise ReleasePublishError(
            f"web archive distribution files differ from the release source: {', '.join(mismatches)}"
        )
...
def _write_evidence(
    ...
    path = output_dir / f"sunodm-{identity.tag}-release-evidence.json"
    payload = {
        "schema_version": 1,
...
def _read_evidence(bundle: PreparedBundle, request: ReleaseRequest) -> dict[str, Any]:
    evidence_path = bundle.output_dir / f"sunodm-{request.tag}-release-evidence.json"
    try:
...
def verify_prepared_bundle(bundle: PreparedBundle, request: ReleaseRequest) -> ReleaseIdentity:
    ...
    evidence_name = f"sunodm-{request.tag}-release-evidence.json"
...
def _build_primary_assets(
    ...
    web_input = input_dir / "sunodm-web-release-validation"
    web_source = _single_file(web_input, ".zip")
    _validate_web_distribution(web_source, source, identity)
    shutil.copyfile(web_source, web)
    sbom_input = _single_file(web_input, ".spdx.json", exact_name=f"sunodm-{identity.sha}.spdx.json")
...
def _build_desktop_assets(input_dir: Path, output_dir: Path, identity: ReleaseIdentity) -> list[Path]:
    ...
    for platform, archive_name in DESKTOP_PREARCHIVES.items():
        source = _only_file(input_dir / f"sunodm-desktop-{platform}-unsigned", archive_name)
        _validate_prearchive(source, platform)
        suffix = ".zip" if platform == "windows" else ".tar.gz"
        destination = output_dir / f"sunodm-{identity.tag}-{platform}-unsigned{suffix}"
