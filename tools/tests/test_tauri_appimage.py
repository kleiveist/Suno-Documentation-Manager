...
def test_windows_portable_output_path_repairs_owner_directory_permissions(
    tmp_path,
) -> None:
...
def test_tauri_linux_build_accepts_explicit_bundle_selection(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: preflight.append(True) or True,
    )
    monkeypatch.setattr(
        common,
        "run_command",
...
def test_tauri_linux_build_uses_appimage_fallback_when_linuxdeploy_fails(
    monkeypatch,
) -> None:
    calls: list[list[str]] = []
    fallback: list[bool] = []
    ...
    monkeypatch.setattr(
        appimage,
        "package_existing_appdir",
        lambda dry_run=False: fallback.append(dry_run) or 0,
    )
...
def test_tauri_linux_build_accepts_fresh_appimage_when_linuxdeploy_returns_failure(
    monkeypatch,
) -> None:
    fallback: list[bool] = []
    output = """
...
def test_tauri_linux_build_does_not_use_appimage_fallback_for_frontend_build_failure(
    monkeypatch,
) -> None:
    fallback: list[bool] = []
...
def test_tauri_build_appimage_shortcut_packages_appdir_on_linuxdeploy_failure(
    monkeypatch,
) -> None:
    fallback: list[bool] = []
    installed: list[bool] = []
...
def test_tauri_build_appimage_shortcut_installs_fresh_appimage_when_linuxdeploy_returns_failure(
    monkeypatch,
) -> None:
    fallback: list[bool] = []
    installed: list[bool] = []
...
def test_tauri_build_appimage_shortcut_does_not_install_on_frontend_build_failure(
    monkeypatch,
) -> None:
    fallback: list[bool] = []
    installed: list[bool] = []
...
def test_tauri_installappimage_module_delegates_to_appimage_installer(
    monkeypatch,
) -> None:
    installed: list[bool] = []
...
def test_tauri_install_appimage_packages_existing_appdir_when_final_file_is_missing(monkeypatch, tmp_path) -> None:
    ...
    def fake_package_existing_appdir(dry_run: bool = False) -> int:
        ...
        (appimage_dir / f"{paths.APP_ARTIFACT_NAME}_1.0.0_amd64.AppImage").write_bytes(b"appimage")
        return 0
...
def test_tauri_build_appimage_dry_run_does_not_install(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        appimage,
        "install_latest",
        lambda dry_run=False: (_ for _ in ()).throw(AssertionError("should not install")),
    )
...
def test_tauri_appimage_command_detection_checks_host_paths(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        appimage,
        "_host_file_exists",
        lambda relative_path: relative_path == "usr/bin/patchelf",
    )
    monkeypatch.setattr(
        common,
        "command_output",
...
def test_tauri_appimage_libfuse_detection_reads_complete_ldconfig_output(
    monkeypatch,
) -> None:
...
def test_tauri_appimage_libfuse_detection_rejects_unrelated_ldconfig_output(
    monkeypatch,
) -> None:
...
def test_tauri_appimage_install_copies_artifact_icon_and_desktop_entry(monkeypatch, tmp_path) -> None:
    ...
    icon_dir.mkdir(parents=True)
    source_appimage = appimage_dir / f"{paths.APP_ARTIFACT_NAME}_1.0.0_amd64.AppImage"
