...
from tools.inst import build, release_publish, release_publish_bundle, release_publish_cli


@pytest.fixture(autouse=True)
def _deny_live_github_requests(monkeypatch: pytest.MonkeyPatch) -> None:
    def fail(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("release publisher tests must not access the network")

    monkeypatch.setattr(release_publish.urllib.request, "urlopen", fail)
...
def _identity(root: Path, sha: str) -> release_publish.ReleaseIdentity:
    return release_publish.validate_release_identity(
        root,
        release_publish.ReleaseRequest("kleiveist/Suno-Documentation-Manager", "v1.0.2", sha, 600, 2),
    )
...
def _run(name: str, run_id: int, sha: str, *, branch: str = "main") -> dict[str, object]:
    ...
    return {
        ...
        "head_sha": sha,
        "head_branch": branch,
        "html_url": f"https://github.com/kleiveist/Suno-Documentation-Manager/actions/runs/{run_id}",
    }
...
def _workflow_evidence(sha: str) -> tuple[release_publish.WorkflowEvidence, ...]:
    ...
    return tuple(
        release_publish.WorkflowEvidence(
            ...
            head_sha=sha,
            url=f"https://github.com/kleiveist/Suno-Documentation-Manager/actions/runs/{100 + index}/attempts/2",
        )
...
def _artifact_inputs(root: Path, sha: str) -> Path:
    inputs = root / "inputs"
    web = inputs / "sunodm-web-release-validation"
    web.mkdir(parents=True)
    with zipfile.ZipFile(web / "sunodm-web.zip", "w") as archive:
        archive.writestr("index.html", b"web candidate")
        for name in release_publish_bundle.WEB_DISTRIBUTION_FILES:
            archive.write(root / "repository" / name, name)
    (web / f"sunodm-{sha}.spdx.json").write_text(
        json.dumps(
            {
    ...
    for platform, archive_name in release_publish_bundle.DESKTOP_PREARCHIVES.items():
        platform_root = inputs / f"sunodm-desktop-{platform}-unsigned"
...
def test_release_identity_requires_annotated_exact_version_tag(tmp_path: Path) -> None:
    ...
    with pytest.raises(release_publish.ReleasePublishError, match="does not match VERSION"):
        release_publish.validate_release_identity(
            root,
            release_publish.ReleaseRequest("kleiveist/Suno-Documentation-Manager", "v1.0.3", sha, 600, 2),
        )


def test_release_workflow_contract_and_cli_surface_are_product_scoped() -> None:
    assert release_publish_bundle.WEB_DISTRIBUTION_FILES == build.WEB_LEGAL_FILES
    assert release_publish.REQUIRED_WORKFLOWS == (
        "SunoDM CI",
        "SunoDM Desktop CI",
        "SunoDM Security",
    )
    assert release_publish.REQUIRED_WORKFLOW_PATHS == {
        "SunoDM CI": ".github/workflows/ci.yml",
        "SunoDM Desktop CI": ".github/workflows/desktop.yml",
        "SunoDM Security": ".github/workflows/security.yml",
    }
    parser = release_publish_cli._parser()
...
def test_collect_workflow_evidence_requires_every_success_on_exact_sha(
    ...
    with pytest.raises(release_publish.ReleasePublishError, match="SunoDM CI"):
...
def test_immutable_release_setting_is_required_and_fail_closed(
    ...
    release_publish.ensure_immutable_releases_enabled("kleiveist/Suno-Documentation-Manager", token="admin-read-token")
    ...
    with pytest.raises(release_publish.ReleasePublishError, match="does not report"):
        release_publish.ensure_immutable_releases_enabled(
            "kleiveist/Suno-Documentation-Manager", token="admin-read-token"
        )
...
def test_build_release_bundle_packages_all_evidence_and_checksums(
    ...
    assert names == {
        "sunodm-v1.0.2-source.zip",
        "sunodm-v1.0.2-web.zip",
        "sunodm-v1.0.2.spdx.json",
        "sunodm-v1.0.2-linux-unsigned.tar.gz",
        "sunodm-v1.0.2-macos-unsigned.tar.gz",
        "sunodm-v1.0.2-windows-unsigned.zip",
        "sunodm-v1.0.2-release-evidence.json",
        "SHA256SUMS.txt",
        "RELEASE_NOTES.md",
    }
    ...
    evidence = json.loads((output / "sunodm-v1.0.2-release-evidence.json").read_text())
    ...
    with zipfile.ZipFile(output / "sunodm-v1.0.2-source.zip") as archive:
        assert "sunodm-v1.0.2/tracked.txt" in archive.namelist()
        assert "sunodm-v1.0.2/untracked-secret.txt" not in archive.namelist()
    with zipfile.ZipFile(output / "sunodm-v1.0.2-web.zip") as archive:
        assert archive.read("LICENSE") == (root / "LICENSE").read_bytes()
        assert archive.read("NOTICE") == (root / "NOTICE").read_bytes()
        assert archive.read("THIRD_PARTY_NOTICES.md") == (root / "THIRD_PARTY_NOTICES.md").read_bytes()
    with tarfile.open(output / "sunodm-v1.0.2-linux-unsigned.tar.gz", "r:gz") as archive:
        member = archive.getmember("nested/candidate-linux.bin")
        assert member.mode & 0o111
    assert release_publish_bundle.sha256(output / "sunodm-v1.0.2-linux-unsigned.tar.gz") == (
        release_publish_bundle.sha256(inputs / "sunodm-desktop-linux-unsigned" / "sunodm-desktop-linux-unsigned.tar.gz")
    )
...
def test_release_bundle_rejects_symlinks_in_downloaded_artifacts(
    ...
    identity = _identity(root, sha)
    inputs = _artifact_inputs(tmp_path, sha)
    target = inputs / "sunodm-desktop-linux-unsigned" / "outside-link"
    try:
...
def test_source_and_web_archives_fail_closed_without_distribution_notices(
    tmp_path: Path,
) -> None:
    root, sha = _release_repository(tmp_path)
    identity = _identity(root, sha)
    source = tmp_path / "incomplete-source.zip"
    with zipfile.ZipFile(source, "w") as archive:
        archive.writestr(
            "sunodm-v1.0.2/LICENSE",
            "# PolyForm Shield License 1.0.0\nhttps://polyformproject.org/licenses/shield/1.0.0\n",
        )
    with pytest.raises(release_publish.ReleasePublishError, match="LICENSE, NOTICE"):
        release_publish_bundle._validate_source_licensing(source, identity)

    inputs = _artifact_inputs(tmp_path, sha)
    web = inputs / "sunodm-web-release-validation" / "sunodm-web.zip"
...
def test_web_distribution_is_bound_to_committed_source_not_dirty_worktree(tmp_path: Path) -> None:
    root, sha = _release_repository(tmp_path)
    identity = _identity(root, sha)
    (root / "NOTICE").write_text("dirty worktree notice\n", encoding="utf-8")
    inputs = _artifact_inputs(tmp_path, sha)

    with pytest.raises(release_publish.ReleasePublishError, match="differ from the release source"):
        release_publish_bundle.build_release_bundle(
            root,
            inputs,
            tmp_path / "release-assets",
...
def test_release_bundle_rejects_invalid_spdx_document(tmp_path: Path) -> None:
    ...
    identity = _identity(root, sha)
    inputs = _artifact_inputs(tmp_path, sha)
    (inputs / "sunodm-web-release-validation" / f"sunodm-{sha}.spdx.json").write_text(
...
def test_release_bundle_rejects_nonportable_archive_members(tmp_path: Path) -> None:
    ...
    identity = _identity(root, sha)
    inputs = _artifact_inputs(tmp_path, sha)
    archive_path = inputs / "sunodm-desktop-linux-unsigned" / "sunodm-desktop-linux-unsigned.tar.gz"
    payload = b"unsafe"
...
def test_prepared_bundle_reverification_rejects_tampering(tmp_path: Path) -> None:
    ...
    web = output / "sunodm-v1.0.2-web.zip"
...
def test_prepared_bundle_rejects_boolean_integer_evidence_fields(
    ...
    evidence_path = output / "sunodm-v1.0.2-release-evidence.json"
