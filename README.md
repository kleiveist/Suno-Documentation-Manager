...
Workflow 1.6 recognizes structured `made with suno studio` metadata in imported Suno WAV exports and records its validated `created` timestamp and `id` as evidence-derived metadata. While a valid metadata date exists, it authoritatively fills and locks the final-generation date, production-end date, and optional download/export date. In Step 07, `No` to desktop-PC editing also derives and locks the last-editing date; `Yes` requires a freely selected date and the confirmed editing work. Manual fallbacks remain available when no valid metadata date exists.

System verification compares SHA-256 values across all verified evidence and explicitly reports when the final release audio is byte-identical to the Suno export. Multiple adjacent global subscription receipts can jointly cover the production period, while any receipt that overlaps production or covers final generation can be attached. Finalized snapshots remain immutable: metadata extraction never backfills them in place, while an explicitly created revision may analyze the carried evidence. Generated documents use template `1.7`; new finalizations write manifest schema `4` and certificate/PDF format `4.1`. These are technical documentation facts only: the result does not certify authorship, ownership, non-infringement, legality, license validity, judicial weight, compliance, or governmental approval.
