...
There is no backend service, remote database, telemetry endpoint, cloud dependency, or required runtime network edge. The only runtime network edges are optional and user-started: one configured timestamp request after the finalization gate has serialized its manifest anchor but before the certificate is rendered, later explicit timestamp retries, and an ACRCloud identification request in Step 07. No provider is contacted during startup, import, replacement, ordinary document/hash generation, verification, or workflow evaluation. Automatic timestamping is part of the user-started finalization transaction only when enabled; provider and Trusted List lookup failures become certificate data and cannot block `DOCUMENTATION COMPLETE`, while successful response bytes are archived before the final commit.
...
Automatic timestamping freezes a `FinalizationTimestampSnapshot` between manifest serialization and the only Markdown/PDF render. It separates provider configuration, concrete timestamp status, protocol/hash/signature/chain checks, and provider-neutral qualification. A successful response uses a two-authority sidecar-v2 transaction: create and verify immutable bytes in contained staging, synchronize the stage and parent, register the certificate-bound row in SQLite, then publish and synchronize it before the final track commit. Later retry and manual/legacy attachment use the same sidecar transaction without changing the base certificate. Manual evidence remains a hash-comparison record and initial OpenTimestamps evidence remains `ATTACHED`.

RFC-3161 signer identity is exposed only after the complete technical predicate succeeds. Qualification matching uses signer-certificate SHA-256 and optional policy/service identity, never provider label or endpoint. A higher eIDAS result additionally requires an already cryptographically validated official Trusted List snapshot, the qualified electronic time-stamp service type, granted status for the relevant period, and retained source/list/service audit fields. Current and timestamp-time qualification remain separate, and absence or lookup failure stays neutral and non-blocking. The architecture does not treat a website query, HTTPS domain, or unvalidated XML as a trust proof.

Workspace recovery completes only matching registered pending state, discards unregistered staging, and rejects an unregistered live sidecar. Load verification requires canonical immutable versioned record bytes and exact published hashes without invoking the current renderer or network. Revision lookup requires `revision.json.previous_certificate.certificateId` to match the sidecar, keeping archived records independently verifiable without folding them into the base certificate result.
...
| Date | Change | Author |
| --- | --- | --- |
| 2026-08-21 | Moved the optional automatic timestamp edge between manifest anchoring and the single certificate render; added sidecar v2 signer identity and provider-neutral Trusted List qualification without making lookup failures blocking. | Project team |
| 2026-08-20 | Added the historical phase-one/phase-two network boundary, fixed-manifest RFC-3161 trust predicate with explicit TSA roots, manual/OTS separation, and PDF/A-2b/full-font certificate architecture. | Project team |
