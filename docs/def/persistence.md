...
Finalization validates all current portable inputs and serializes the schema-9 Evidence Manifest first. Its SHA-256 becomes the immutable timestamp anchor. If automatic timestamping is enabled, one bounded provider attempt runs against that anchor before the Markdown and both format-6.2 PDFs are rendered once with the captured provider-configuration, concrete technical, protocol, signer-identity, and qualification state. The certificate set is staged, published, and verified; an exact successful provider response is then archived as a sidecar and verified before the `FINALIZED` database commit. Provider/network/qualification lookup failures are non-blocking captured data. A local certificate or sidecar staging, integrity, publication, or database failure remains transactional and rolls back the correlated application-created artifacts. An incomplete set must not be presented as finalized.
...
### Finalization timestamp and later attachment

The automatic attempt is embedded in the single-render finalization transaction; later explicit retry and manual/legacy attachment remain post-finalization operations. Neither path is ordinary track-evidence import. The mandatory documentation gate ends before the optional network/trust checks, so a provider outage, malformed response, missing qualification source, or unavailable historical Trusted List status does not block `DOCUMENTATION COMPLETE` or turn into a legal conclusion.

The automatic RFC-3161 path has one non-user-selectable anchor and verification contract:

1. Require the native finalization gate to pass and freeze the track/profile/workflow/evidence snapshot plus Certificate ID and finalization-snapshot ID.
2. Serialize `06_CERTIFICATE/EVIDENCE_MANIFEST.json` schema 9 and calculate its exact SHA-256 before any provider call. The UI and provider cannot substitute another artifact or digest.
3. Read the current non-secret provider settings and private credential, then make at most one bounded request containing the manifest SHA-256, a fresh random nonce, and the optional custom policy OID. Disabled or unusable configuration produces a concrete `NOT RECORDED` state; request/response failure produces a concrete technical failure state rather than aborting finalization.
4. Accept a positive response only when TimeStampResp status is `granted` or `grantedWithMods`, TSTInfo is version 1, the SHA-256 message imprint and nonce match exactly, a policy OID is returned, and a configured requested policy matches.
5. Cryptographically verify the CMS signed attributes and signature, strict timestamping EKU, signer validity at `genTime`, and chain to explicitly configured TSA trust anchors. Only after these checks does the adapter expose signer-certificate DER, certificate SHA-256, subject, issuer, serial, policy, and service identity for provider-neutral qualification matching.
6. When an already cryptographically validated official Trusted List snapshot is available, match by verified signer-certificate SHA-256 and optional policy/service identity, evaluate the service status separately at `genTime` and at the qualification-check time, and retain the exact list/service audit fields. Provider label, endpoint, URL, domain, and self-description are never classifier inputs.
7. Freeze one `FinalizationTimestampSnapshot`, render the Markdown and both final PDFs exactly once, publish and verify the certificate set, and archive the same untouched successful provider bytes plus sidecar-v2 audit artifacts before the final database commit. No second provider request is made. A later retry can create another immutable sidecar but cannot rewrite the final PDF.
...
Qualification is a separate immutable audit record. Its statuses distinguish not checked, not documented, not verified, verified provider identity, verified Trust Service, verified qualified service, and check failure. It records signer-certificate identity, Trust Service identity, source/list version/sequence/hash/validation, service type, and separate service-status URI/period evidence plus qualification results for timestamp time and check time. A positive qualified-service claim is rejected at PDF validation unless the list evidence is `VERIFIED`, the signer identity is present and consistent, and the official qualified timestamp service type and granted status support every positively reported time axis. The application currently consumes only an already cryptographically validated Trusted List snapshot; it does not promote a web search, HTTPS endpoint, provider label, or unvalidated downloaded XML into qualification evidence.
...
Whenever a timestamped track is loaded, every registered current or archived record is independently resolved and sidecar-reverified. Reverification requires the exact managed regular-file set described by the record; byte-for-byte equality with the canonical immutable versioned JSON registered by SQLite; SHA-256 verification of the exact published record, evidence, optional raw provider response, Markdown, and PDF bytes; equality with pinned provider-response/Markdown/PDF hashes; an unchanged referenced phase-one or archived artifact with the correct stored match result; and an exact versioned `TIMESTAMP_RECORD_SHA256.txt`. It deliberately hashes published bytes instead of re-rendering historical addenda. An archived location additionally requires `revision.json.previous_certificate.certificateId` to equal the sidecar Certificate ID.

Load-time sidecar verification does not silently rerun a network request or reinterpret legacy evidence. For an automatic RFC-3161 record, the positive summary is reconstructed only when the exact immutable verified provider metadata is still covered by the intact sidecar, the referenced artifact is specifically the finalized Evidence Manifest with equal claimed/actual SHA-256, nonce and policy predicates remain complete, signature and pinned-chain results are positive, the strict versioned verifier identifier matches, and all retained trust-anchor fingerprints are well formed. Missing, stale, tampered, or merely summary-only `VERIFIED` state is downgraded to a visible failure. A damaged sidecar remains associated with its record but does not rewrite or invalidate the independent base certificate. Legacy format-v0 and v1 sidecars remain verifiable under their historical contracts and are not silently rewritten as v2.
...
| `REQ-PER-010` | Schema 5 persists certificate-bound timestamp records; schema 6 adds non-secret provider settings while secrets stay outside SQLite. Automatic finalization timestamps the serialized manifest before the one-time certificate render and archives the same response as sidecar v2 before the final database commit. Concrete `VERIFIED` requires the complete RFC-3161 predicate; a higher qualification additionally requires verified signer/service identity and a cryptographically validated official Trusted List record for the relevant period. Provider/trust lookup failures remain non-blocking, and manual/legacy/OTS evidence is not promoted. | [ATP-0016](../atp/active/ATP-0016-evidence-certificate-workflow-5.md) |
...
| Date | Change | Author |
| --- | --- | --- |
| 2026-08-21 | Embedded one automatic manifest-anchored provider attempt before the single certificate render, added sidecar v2 provider-neutral qualification audit data, and kept provider/trust failures non-blocking before the final commit. | Project team |
| 2026-08-20 | Defined the historical post-commit fixed-manifest RFC-3161 path, complete cryptographic verification predicate with explicit TSA trust anchors, manual/legacy/OTS separation, and exact sidecar-based reconstruction of current status. | Project team |
