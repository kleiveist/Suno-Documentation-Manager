...
└── 06_CERTIFICATE/
    ...
    └── EXTERNAL_TIMESTAMPS/              # optional, automatic success or later attachment
        └── <timestamp-record-id>/
            ├── TIMESTAMP_RECORD.json      # immutable sidecar format v2
...
| External timestamp evidence | `06_CERTIFICATE/EXTERNAL_TIMESTAMPS/<timestamp-record-id>/` | Optional certificate-bound sidecar for a successful automatic finalization request or a later explicit attachment; automatic RFC-3161 uses only the serialized Evidence Manifest anchor, while manual/legacy records may bind another permitted stable artifact; qualification remains a separate externally verified audit result |
...
- root-level `SunoDM_DOCUMENTATION_CERTIFICATE.pdf` (English) and `SunoDM_DOCUMENTATION_CERTIFICATE_DE.pdf` (German), certificate format `6.2`, as deterministic PDF/A-2b documents with A–L sections for identity; the separated instrumental, vocal, and Suno-field facts; every source branch; selected human contribution; separate Audio and Artwork AI assessments; license/Terms/technical-coverage facts; the full evidence register; integrity anchors; K.2 audio-screening summary; locally recorded earlier revision-archive references; and Section I provider configuration, concrete timestamp, protocol verification, signer identity, and provider qualification;
- `DOCUMENTATION_CERTIFICATE.md`, certificate format `6.2`, with the same factual scope, evidence register, origin labels, status meanings, K.2 screening summary, Section I timestamp/trust detail, and explicit non-legal boundary;
- `EVIDENCE_MANIFEST.json` schema `9`, including the complete `documented_facts` and `profile_snapshot`, fact-origin and answer-state definitions, separated lyrics and AI assessment values, Suno detection summary, generic byte-identical pairs, automatic role relationships, consistency results, a sanitized `audioScreening` section, the certificate PDF archive/font metadata, statement scope, full evidence metadata/lineage, and the manifest-anchor/single-render timestamp ordering; and
...
## External timestamp and provider qualification

The base format-6.2 certificate records the actual timestamp state captured at its only generation. The mandatory gate first freezes the track snapshot and serializes schema-9 `EVIDENCE_MANIFEST.json`; the manifest SHA-256 is then available as a stable anchor. When automatic timestamping is enabled, exactly one provider attempt runs against that anchor before the Markdown and both PDFs are rendered. Disabled configuration, provider failure, failed concrete verification, or unavailable qualification data is represented in the final certificate without blocking `DOCUMENTATION COMPLETE`. A successful response is archived from the same bytes as an immutable sidecar before the final database commit. Later retry/manual attachment remains possible but never rewrites the historical PDF.

Section I and the frontend answer four independent questions: provider readiness; whether concrete timestamp evidence was created; the protocol/imprint/signature/chain/identity checks for that concrete evidence; and the externally verified trust or qualification of the signer service. RFC 3161 and OpenTimestamps describe technical mechanisms, not regulatory qualification. Technical `VERIFIED` with qualification `NOT VERIFIED` is valid, as is a known qualified provider with a failed concrete request.

For RFC 3161, the adapter exposes the signer certificate identity only after successful CMS/EKU/time/chain verification. Qualification matching uses certificate SHA-256 plus optional policy/service identity and never the configured provider label, endpoint, URL, or domain. A positive eIDAS result requires an already cryptographically validated official Trusted List snapshot, the exact qualified electronic time-stamp service type, and granted service status for the relevant period. The audit retains source, territory, version, sequence, issue/next-update times, list SHA-256 and validation status/time, Trust Service Provider/service/type/identifier, and separate service-status URI/period evidence plus qualification results for timestamp time and check time. Missing/unreachable/unvalidated data remains neutral or `CHECK FAILED`; it is not a finding that the provider is unsafe or unqualified. The current architecture does not treat a website query or unvalidated downloaded XML as qualification proof.
...
Sidecar format v2 follows a durable stage -> database registration -> live publication sequence. Historical v1 and v0 records remain readable under their original contracts and are not rewritten. The core set contains immutable `TIMESTAMP_RECORD.json`, managed timestamp evidence, Markdown addendum, PDF/A-2b addendum, and `TIMESTAMP_RECORD_SHA256.txt`; an adapter may additionally retain an exact raw provider response when the evidence is a derived wrapper. The staged file set and parent are synchronized before its certificate-bound SQLite row is created, using directory `fsync` where supported. Only a registered record is published. Recovery publishes matching registered stages, removes abandoned unregistered stages, and rejects unexpected unregistered live directories.

The immutable record pins its Certificate ID and finalization-snapshot ID, selected anchor, evidence and optional raw-response hashes, RFC request/response verification facts, signer-certificate identity, provider-neutral qualification/Trusted List audit when applicable, publication-time integrity, and exact Markdown/PDF hashes. It excludes current `integrityVerified` and `integrityIssues`. The hash list protects every managed sidecar artifact except itself. The anchored manifest, main hash list, Markdown certificate, both PDF certificates, and certificate hash list are never regenerated, so the stamped anchor remains byte-identical and no self-reference cycle exists.

On every load, the application resolves each registered current or archived record and checks the exact regular-file set, byte-identical canonical JSON registered by SQLite, every sidecar hash, pinned response/Markdown/PDF hashes, referenced phase-one artifact and stored match result, and archive-to-Certificate-ID binding. It hashes the published addendum bytes instead of re-rendering them. For an automatic RFC-3161 record, the UI reconstructs technical `VERIFIED` only if that intact sidecar still contains the complete positive provider predicate, is bound specifically to an equal claimed/actual Evidence Manifest anchor, uses the exact current verifier identifier, and retains well-formed pinned-root fingerprints. A qualified-service badge additionally requires internally consistent signer identity, cryptographically validated Trusted List evidence, official service type/status, and the relevant timestamp-time result. It does not rerun a network request or promote manual, legacy, OTS, incomplete, or stale summary state. Sidecar damage changes only that record's current integrity/status; the independent base certificate remains unchanged.
...
| `REQ-CER-002` | Format 6.2 renders one final certificate from a schema-9 manifest anchor plus one captured automatic provider attempt. Concrete technical and provider-qualification states remain independent; a positive eIDAS result requires verified signer identity and cryptographically validated official Trusted List service/status evidence for the relevant period. Successful responses and later attachments use immutable sidecar v2, while legacy/manual/OTS records remain distinct and no later record rewrites the base certificate or transfers to another revision. | [ATP-0016](../atp/active/ATP-0016-evidence-certificate-workflow-5.md) |
...
| Date | Change | Author |
| --- | --- | --- |
| 2026-08-21 | Advanced manifest to 9, certificate/PDF to 6.2, and sidecar to 2 for one-shot finalization timestamp state, signer identity, provider-neutral Trusted List qualification, and separate historical/current status. | Project team |
