...
| --- | --- |
| Status | Active |
| Decision status | Gate A and the five-item Gate B decision complete; ownership and technical matrix passed; Gate C technical commit decision and adoption still open |
...
| `tools/config/**`, `tools/profiles/**`, other `tools/inst/**`, `tools/tauri/**` | INTEGRATION | TEMPLATE_MANAGED | Existing product-shaped shared tooling | v1.0.3 baseline | Controlled merge | Take generic fixes; retain SunoDM names, sidecars, commands | Tool or bundle regression | Tools suite and dry runs |
