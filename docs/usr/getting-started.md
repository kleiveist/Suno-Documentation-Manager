...
Under `Einstellungen` → `Externe Dienste`, enable the external timestamp service only when you want phase-two network access. Select FreeTSA, Sigstore Public TSA, OpenTimestamps, or a custom RFC-3161 service. `Automatisch nach Finalisierung` means that a user-started finalization first completes and commits the immutable local certificate, then starts the optional provider action. A provider failure cannot undo finalization.
...
Automatic RFC-3161 attachment always timestamps the exact SHA-256 of finalized `06_CERTIFICATE/EVIDENCE_MANIFEST.json` with a fresh nonce. A positive record additionally requires the response policy contract, CMS signature, critical sole timestamping EKU, certificate validity at `genTime`, and a chain to the configured trust anchor. OpenTimestamps is a separate proof format: its initial result is stored as `ATTACHED` until separately verified or upgraded and is not labeled as RFC-3161 verification. See [Finalizing a track](finalizing-a-track.md#attach-optional-external-timestamp-evidence) before enabling the automatic action.
