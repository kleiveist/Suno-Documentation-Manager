...
Setting 04, `Zertifikatssprache`, is also the app language. Select `Deutsch` or `Englisch` and save the settings; navigation, workflow labels, dialogs, and other UI copy switch to the selected language. This affects future finalizations only; existing certificate and track snapshots remain unchanged. Step 10 creates both PDF/A-2b certificate PDFs automatically: one in German and one in English, with the complete bundled DejaVu 2.37 fonts embedded.
...
## Optional external timestamp settings

Under `Einstellungen` → `Externe Dienste`, enable the external timestamp service only when you want optional network access during a user-started finalization or a later explicit retry. Select FreeTSA, Sigstore Public TSA, OpenTimestamps, or a custom RFC-3161 service. `Automatisch während der Finalisierung` means that SunoDM first serializes the immutable Evidence Manifest anchor, makes exactly one bounded provider attempt, and then renders the Markdown and both final PDFs once with that actual result. A disabled, unavailable, or failed provider is recorded as a non-positive timestamp state and does not block `DOCUMENTATION COMPLETE`.

Every RFC-3161 provider requires an explicit local TSA CA trust-anchor file before SunoDM can report `VERIFIED`; the app does not silently substitute the operating-system trust store or download a root. A custom service can additionally define its endpoint, optional policy OID, timeout, and supported authentication settings. Keep credentials in the local secret fields; they do not enter a track, manifest, certificate, sidecar, or portable folder. `Provider testen` checks configuration and provider response handling, but it has no finalized artifact bytes and therefore is not proof that a track timestamp's CMS signature and chain were verified.

The automatic RFC-3161 attempt always timestamps the exact SHA-256 of `06_CERTIFICATE/EVIDENCE_MANIFEST.json` with a fresh nonce. A positive technical result additionally requires the response policy contract, CMS signature, critical sole timestamping EKU, certificate validity at `genTime`, and a chain to the configured trust anchor. The signer certificate identity is recorded independently from the freely configured provider label or endpoint. RFC 3161 is a protocol, not an eIDAS qualification: a higher qualification is shown only when that verified signer/service identity is matched to a cryptographically validated official Trusted List snapshot for the relevant time. Without such evidence the status remains neutral (`NOT CHECKED`, `NOT DOCUMENTED`, or `NOT VERIFIED`), never an inferred negative finding. OpenTimestamps remains a separate proof format and receives no automatic regulatory classification. See [Finalizing a track](finalizing-a-track.md#attach-optional-external-timestamp-evidence) before enabling the automatic action.
...
Under `Einstellungen` → `Externe Dienste` → `Pre-release audio screening`, enter the ACRCloud host, enable the provider, and save the access key and access secret. The fields are write-only: after saving, the UI can report only whether a complete credential pair exists. Use `Provider testen` to check the configured endpoint; this does not upload track audio.

The **ACRCloud screening intensity** area below the documentation link controls the requested sampling coverage. Choose the percentage, then choose either dynamic calculation from the actual track duration or a fixed reference duration. The preview shows the requested sample time, expected request count, and the maximum unique audio duration that can actually be sampled. ACRCloud samples are at most 12 seconds each; SunoDM never sends more than 25 requests or 300 seconds of unique audio for one track. High and very-high levels show an API-usage warning.

Then open Step 07 and choose the external screening action for one specific track. It sends a deterministic set of distinct, evenly distributed bounded release-audio samples only after that click, never the local Chromaprint fingerprint. If the track is too short for another non-overlapping interval, SunoDM reduces the request count. The action is optional and cannot block finalization.
...
In `04 Human Work`, keep Suno Instrumental Mode, Vocal Intent, Content Classification, and the actual final-audio result separate. Choose exactly one classification: `STRUCTURE_ONLY`, `VOCAL_LYRICS_ONLY`, `MIXED`, `EMPTY`, or `OTHER`. Vocal Lyrics plus structure instructions use `MIXED`. `EMPTY` needs no source or text; every other value requires the exact text and its `human`, `AI`, or `mixed` source, and `OTHER` also needs a factual label. Choose Vocal Intent explicitly as `VOCAL`, `INSTRUMENTAL`, or `UNSPECIFIED`. It is not derived from any other answer, and a difference between intent and the final audio is not a blocker.

Enter the complete Suno style prompt and select only the human-editing steps that actually occurred. Suno structure text is not automatically Human Work; its documented content source controls that statement. If post-export editing is `Yes`, select at least one actual post-export operation. Document generation writes `02_SUNO/Lyrics.md` under the neutral `Suno Generation Text Field` heading and `02_SUNO/Style.md`.
...
The application copies evidence; it does not move or delete the source. A normal track import receives `managed_copy` provenance. New artwork copies use an ASCII-uppercase track stem with `_` separators and the suffixes `_AI_ORIGINAL`, `_AI_EDITED`, `_HUMAN_EDITED`, and `_FINAL`; for example, `My Track` becomes `MY_TRACK_HUMAN_EDITED.png`. This convention applies only to new imports and replacements. Legacy `_EDITED`, mixed-case, and hyphenated paths remain recognized, and finalized evidence is never renamed. It never silently replaces an existing destination. If a same-name file already exists without a matching indexed record, resolve the conflict explicitly instead of assuming that the files are identical. Artwork import timestamps record only the import into SunoDM, not the actual creation or editing order of the source files.
...
| Date | Change | Author |
| --- | --- | --- |
| 2026-08-21 | Updated timestamp settings for one automatic manifest-anchored request before the single certificate render and separate signer/Trusted-List qualification reporting. | Project team |
| 2026-08-20 | Added historical external timestamp configuration guidance for the post-commit fixed-manifest RFC-3161 path, explicit TSA trust anchors, provider tests, and distinct OpenTimestamps status; noted PDF/A-2b/full DejaVu certificate output. | Project team |
