<!-- AUTO-GENERATED:backlink START -->
[← Back](../README.md)
<!-- AUTO-GENERATED:backlink END -->
# Docs

## Product documentation

- [Application architecture](def/architecture.md)
- [Detailed product architecture](def/product-architecture.md)
- [Track library organization model](def/track-library-model.md)
- [Track documentation model](def/track-documentation-model.md)
- [Local persistence and recovery](def/persistence.md)
- [Suno track workflow model](def/workflow-model.md)
- [Getting started](usr/getting-started.md)
- [Finalizing a track](usr/finalizing-a-track.md)
- [Legacy track import](dev/legacy-track-import.md)
- [Product acceptance records](atp/active/active.md)

Generic template, backend, CI, database, container, and deployment pages retained in the generated index are marked as inherited reference or unavailable. They do not override the product documentation above.

<!-- AUTO-GENERATED:docs-index START -->

## 📄 Pages
- 📝 [<Document title>](DOCUMENT-TEMPLATE.md)

## 📁 ATP
- 🗂️ [Overview](atp/atp.md)
- 📝 [ATP-<ID>: <Acceptance title>](atp/ATP-TEMPLATE.md)

## 📁 DEF
- 🗂️ [Overview](def/def.md)
- 📝 [Application architecture](def/architecture.md)
- 📝 [Runtime configuration — inherited template reference](def/configuration.md)
- 📝 [Database feature — unavailable inherited reference](def/database-feature.md)
- 📝 [Deployment architecture — unavailable inherited reference](def/deployment-architecture.md)
- 📝 [Provider-neutral persistence architecture — inherited template reference](def/persistence-architecture.md)
- 📝 [Local persistence and recovery](def/persistence.md)
- 📝 [Pre-release audio screening](def/pre-release-audio-screening.md)
- 📝 [Suno Documentation Manager product architecture](def/product-architecture.md)
- 📝 [Project profiles — inherited template reference](def/project-profiles.md)
- 📝 [Track documentation model](def/track-documentation-model.md)
- 📝 [Track library organization model](def/track-library-model.md)
- 📝 [Suno track workflow model](def/workflow-model.md)

## 📁 DEV
- 🗂️ [Overview](dev/dev.md)
- 📝 [Acceptance execution report — 2026-08-14](dev/acceptance-report.md)
- 📝 [Certificate PDF architecture](dev/certificate-pdf-architecture.md)
- 📝 [Codex CLI prompt for folder-import preparation](dev/folder-import-preparation-prompt.md)
- 📝 [Legacy track import and managed-document adoption](dev/legacy-track-import.md)
- 📝 [Upstream template final acceptance — historical reference](dev/template-final-acceptance.md)

## 📁 Tools
- 🗂️ [Overview](tools/tools.md)
- 📝 [Continuous integration — unavailable inherited reference](tools/ci.md)
- 📝 [Container builds and local production simulation — unavailable inherited reference](tools/container-builds.md)
- 📝 [Release and desktop packaging model](tools/release-model.md)
- 📝 [Tooling guide](tools/tooling.md)

## 📁 USR
- 🗂️ [Overview](usr/usr.md)
- 📝 [Finalizing a track](usr/finalizing-a-track.md)
- 📝 [Getting started with Suno Documentation Manager](usr/getting-started.md)

<!-- AUTO-GENERATED:docs-index END -->
