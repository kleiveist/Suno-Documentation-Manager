...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `REQ-WFL-001` | Load the declarative workflow. | ID `suno-track`, version `1.6`, and exactly ten uniquely ordered required steps load; unsupported schema or duplicate step is rejected, and the download/export date remains optional. | The current configuration fixture asserts embedded workflow 1.6, ten ordered steps, schema 1, derived optional download/last-editing behavior, and rejection of unsupported schema, empty/duplicate step IDs, unknown kinds, missing fields, and a missing mandatory step. | PASS | Rust `valid_version_1_6_configuration_is_accepted` and workflow configuration tests |
