...
| --- | --- | --- | --- | --- | --- |
| 01 | Instrumental track with `[Intro]`, `[Drop]`, `[Outro]`, no vocal lyrics, and structure content | Workflow passes the lyrics consistency requirement; PDF says instrumental `YES`, vocal lyrics `NO`, and structure instructions present without a vocal claim | Rust `instrumental_structure_content_is_valid_but_instrumental_vocals_conflict`, `renders_clean_instrumental_lyrics_statement_and_scope_boundary`, `explicit_vocal_and_structure_fields_render_without_legacy_inference`; frontend `TEST 01 accepts an instrumental with bracketed structure instructions and no vocal lyrics` | Mapped Rust and frontend checks passed in the full suites | PASS |
| 02 | Instrumental `YES` and vocal lyrics `YES` | Native/UI workflow blocks until the explicit contradiction is corrected; free text is not parsed to decide it | Rust `instrumental_structure_content_is_valid_but_instrumental_vocals_conflict`; frontend `TEST 02 blocks explicit vocal inconsistencies without parsing free text` | Mapped Rust and frontend checks passed in the full suites | PASS |
| 03 | Vocal track with explicit vocal content | Valid workflow result and `Vocal Lyrics` PDF rendering from the new fields | Rust `vocal_track_and_other_suno_field_content_require_only_the_new_explicit_facts`, `renders_vocal_track_from_explicit_new_fields`; frontend `TEST 03 accepts a vocal track with vocal lyrics` | Mapped Rust and frontend checks passed in the full suites | PASS |
