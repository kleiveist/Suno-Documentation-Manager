...
| --- | --- | --- |
| TD-01 | Finalized revision | Valid current workflow `suno-track` `1.4` certificate set |
...
| TD-04 | Workflow update | Recognized test-only workflow version `1.5`, newer than finalized `1.4` |
...
| 6 | `REQ-WFL-006` | Compare archived files to recorded TD-01 digests. | Archived certificate artifacts match the prior finalized bytes exactly. | Native revision and workflow-upgrade fixtures snapshot all current certificate artifacts, including the PDF, and compare the archived main hash list byte for byte. | PASS | Rust `finalized_track_rejects_mutation_until_revision_archives_snapshot`; `workflow_upgrade_archives_finalized_v14_and_requires_fresh_v15_outputs` |
| 7 | `REQ-WFL-006` | Inspect current integrity inclusion after revision. | `.archive/` is excluded and the new working state requires fresh documents and hashes. | Explicit reevaluation archived the finalized snapshot, returned the working track to `ACTIVE`, marked documents stale, cleared generated/verified integrity and current certificate state, and removed the working SHA-256 list while preserving the archive. | PASS | Rust `workflow_upgrade_archives_finalized_v14_and_requires_fresh_v15_outputs` |
...
| 9 | `REQ-WFL-005` | Load TD-01 with TD-04 current. | UI shows finalized workflow `1.4` and current test workflow `1.5`; old certificate bytes and meaning do not change. | Frontend presentation tests expose explicit reevaluation for an older workflow without mutating certificate state; the native fixture snapshots the 1.4 certificate bytes before action. | PASS | Frontend `workflowUpgradePresentation` tests; Rust workflow-upgrade fixture |
| 10 | `REQ-WFL-005` | Choose explicit reevaluation under TD-04. | A new working revision is created; a new certificate requires all workflow 1.5 requirements and never rewrites archived `1.4`. | Native explicit reevaluation from `1.4` to test workflow `1.5` creates one revision archive with exact old certificate/hash bytes, resets current outputs and status, and rejects duplicate reevaluation. | PASS | Rust `workflow_upgrade_archives_finalized_v14_and_requires_fresh_v15_outputs` |
...
cargo test workflow_upgrade_archives_finalized_v14_and_requires_fresh_v15_outputs
cd ../frontend
