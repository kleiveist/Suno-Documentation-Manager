...
| --- | --- | --- |
| TD-01 | Finalized revision | Valid current workflow `suno-track` `1.6` certificate set |
...
| TD-04 | Workflow update | Recognized test-only workflow version `1.7`, newer than finalized `1.6` |
...
| 3 | `REQ-WFL-006` | Inspect certificate artifacts immediately after mismatch. | Original certificate Markdown, manifest, certificate hash file, and root-level PDF remain byte-identical; none is silently regenerated. | Certificate-set byte snapshots, including the technical PDF, remain unchanged when external evidence invalidates the presentation. | PASS | Rust `external_change_invalidates_certificate_state_without_rewriting_certificate_files` |
...
| 5 | `REQ-WFL-006` | Choose `Create new revision` after TD-02. | A unique `.archive/revisions/<revision-id>/` is created and contains the prior certificate directory, root-level PDF, main hash list, and revision metadata. | The native revision action created one unique revision directory containing `revision.json`, the three files from `06_CERTIFICATE/`, the technical PDF, and the prior main hash list. | PASS | Rust `finalized_track_rejects_mutation_until_revision_archives_snapshot`; [final suite](../../../.report/test-report-20260813-232332-suite-all-ok.md) |
| 6 | `REQ-WFL-006` | Compare archived files to recorded TD-01 digests. | Archived certificate artifacts match the prior finalized bytes exactly. | Native revision and workflow-upgrade fixtures snapshot all current certificate artifacts, including the PDF, and compare the archived main hash list byte for byte. | PASS | Rust `finalized_track_rejects_mutation_until_revision_archives_snapshot`; `workflow_upgrade_archives_finalized_v16_and_requires_fresh_v17_outputs` |
| 7 | `REQ-WFL-006` | Inspect current integrity inclusion after revision. | `.archive/` is excluded and the new working state requires fresh documents and hashes. | Explicit reevaluation archived the finalized snapshot, returned the working track to `ACTIVE`, marked documents stale, cleared generated/verified integrity and current certificate state, and removed the working SHA-256 list while preserving the archive. | PASS | Rust `workflow_upgrade_archives_finalized_v16_and_requires_fresh_v17_outputs` |
...
| 9 | `REQ-WFL-005` | Load TD-01 with TD-04 current. | UI shows finalized workflow `1.6` and current test workflow `1.7`; old certificate bytes and meaning do not change. | Frontend presentation tests expose explicit reevaluation for an older workflow without mutating certificate state; the native fixture snapshots the 1.6 certificate bytes before action. | PASS | Frontend `workflowUpgradePresentation` tests; Rust workflow-upgrade fixture |
| 10 | `REQ-WFL-005` | Choose explicit reevaluation under TD-04. | A new working revision is created; a new certificate requires all workflow 1.7 requirements and never rewrites archived `1.6`. | Native explicit reevaluation from `1.6` to test workflow `1.7` creates one revision archive with exact old certificate/hash bytes, resets current outputs and status, and rejects duplicate reevaluation. | PASS | Rust `workflow_upgrade_archives_finalized_v16_and_requires_fresh_v17_outputs` |
...
cargo test workflow_upgrade_archives_finalized_v16_and_requires_fresh_v17_outputs
cd ../frontend
...
- Summary: Steps 2–7, 9–11, and 13–14 passed; three mandatory steps remain `NOT RUN`.
