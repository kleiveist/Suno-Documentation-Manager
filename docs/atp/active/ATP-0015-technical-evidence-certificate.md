...
| 5 | Register local Suno Terms once in Settings | Global metadata survives restart; a hashed `global_copy` is automatically placed in every new/editable project, and Manifest/PDF retain source metadata, path, provenance, size, import time, and full hash | SQLite schema-4 migration, global metadata round-trip, existing/new-track propagation, manifest model, and PDF terms fixture implemented | PASS | Rust `global_terms_and_track_timestamp_imports_persist_factual_metadata`, migration tests, and PDF terms fixture |
