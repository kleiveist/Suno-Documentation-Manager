...
| --- | --- | --- | --- | --- | --- |
| 1 | Vocal Intent = `INSTRUMENTAL`, final audio contains vocals = `YES` | Native finalization accepts the two explicitly recorded facts without correction or inference | Native and frontend independence fixtures cover this cross-product | PASS | Rust `vocal_intent_classification_and_final_audio_are_independent`; Vitest `keeps Vocal Intent, classification, instrumental mode, and final audio independent` |
| 2 | Content Classification = `EMPTY`, Vocal Intent explicitly selected | Generated documents print `EMPTY`, the explicit Vocal Intent, and `N/A` for text/source details without inventing lyrics | Renderer, normalization, and workflow fixtures assert the branch | PASS | Rust `other_requires_a_label_while_empty_makes_content_details_not_applicable`; Vitest EMPTY normalization and workflow fixtures |
...
| 11 | Import WAVs with one valid structured `made with suno studio` or `made with suno` record | The exact structured value, single valid RFC 3339 `created` timestamp/date, single UUID `id`, technical audio facts, evidence ID, and SHA-256 are persisted as evidence-derived metadata; duplicate/mixed/incidental markers are rejected, and the applicable dates are authoritatively filled and read-only | Focused parser, evidence-capture, application, revision, and frontend fixtures cover both exact marker variants, the unchanged structural boundary, native override protection, immutable finalized bytes, and manual fallback without metadata | PASS | Rust `reads_pcm_properties_and_suno_info_comment`, `reads_exact_suno_marker_alias_without_relaxing_structured_fields`, `automatic_wav_metadata_accepts_exact_suno_marker_alias_and_drops_unsafe_text`, `p0_suno_marker_alias_import_persists_metadata_and_derives_authoritative_dates`, `p0_finalized_marker_alias_is_immutable_until_revision_reanalysis`; Vitest metadata automation fixtures |
...
| 15 | Open a finalized pre-metadata snapshot, then explicitly create a revision | Loading/listing/reopening leaves database rows and certificate bytes unchanged; only the new mutable revision analyzes the hash-matching carried WAV | The focused alias fixture asserts exact JSON rows, timestamps, facts, origins, and certificate bytes before/after load, verifies archived certificate bytes, and then checks revision analysis | PASS | Rust `p0_finalized_marker_alias_is_immutable_until_revision_reanalysis` |
