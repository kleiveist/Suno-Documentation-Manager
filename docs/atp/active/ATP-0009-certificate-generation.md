...
| Current target | Workflow `1.4`; document template `1.6`; manifest schema `3`; certificate/PDF format `4.0` |
...
| --- | --- | --- |
| TD-01 | Ready track | Artist `Acceptance Artist`, track `Acceptance Track`, workflow `suno-track` `1.4`, concrete final generation, fixed evidence roles |
...
| 3 | `REQ-CER-001` | Review certificate fields. | Certificate format `4.0`, certificate ID, track, artist, workflow ID/version, app version, timestamp, mandatory result, TD-02 reasons, evidence count, selected hashes, blocking deviations, compact final-Suno origin/metadata/byte-identity facts, and `DOCUMENTATION COMPLETE` are present. | Not run | NOT RUN | — |
...
| 5 | `REQ-CER-001` | Parse `EVIDENCE_MANIFEST.json`. | Valid JSON contains `schema_version: 3`, full `documented_facts`/`profile_snapshot`, origin labels, statement scope, workflow/finalization, complete evidence metadata/lineage, hashes, certificate objects, Suno metadata/origins, byte-identical pairs, automatic role relationships, and consistency issues; global evidence exposes its source record ID plus concrete coverage dates. | Not run | NOT RUN | — |
