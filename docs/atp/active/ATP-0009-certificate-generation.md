...
| --- | --- | --- |
| TD-01 | Ready track | Artist `Acceptance Artist`, track `Acceptance Track`, workflow `suno-track` `1.3`, concrete final generation, fixed evidence roles |
...
| 5 | `REQ-CER-001` | Parse `EVIDENCE_MANIFEST.json`. | Valid JSON contains `schema_version: 2`, full documented facts/profile snapshot, origin labels, statement scope, workflow/finalization, evidence metadata/lineage, hashes, and certificate objects; global evidence exposes its source record ID plus concrete coverage dates. | Not run | NOT RUN | — |
