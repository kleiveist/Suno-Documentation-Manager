...
| Current target | Workflow `1.6`; document template `1.7`; manifest schema `4`; certificate/PDF format `4.1` |
...
| --- | --- | --- |
| TD-01 | Ready track | Artist `Acceptance Artist`, track `Acceptance Track`, workflow `suno-track` `1.6`, concrete final generation, fixed evidence roles |
...
| 3 | `REQ-CER-001` | Review certificate fields. | Certificate format `4.1`, certificate ID, track, artist, workflow ID/version, app version, timestamp, mandatory result, TD-02 reasons, evidence count, selected hashes, blocking deviations, compact date-origin/metadata/byte-identity facts, and `DOCUMENTATION COMPLETE` are present. | Not run | NOT RUN | — |
...
| 5 | `REQ-CER-001` | Parse `EVIDENCE_MANIFEST.json`. | Valid JSON contains `schema_version: 4`, full `documented_facts`/`profile_snapshot`, origin labels, statement scope, workflow/finalization, complete evidence metadata/lineage, hashes, certificate objects, all derived date origins, joint subscription coverage, byte-identical pairs, automatic role relationships, and consistency issues; global evidence exposes its source record ID plus concrete coverage dates. | Not run | NOT RUN | — |
...
| 8 | `REQ-CER-001` | Inspect `CERTIFICATE_SHA256.txt`. | It hashes the main hash list, manifest, certificate Markdown, and root-level technical PDF; it has no self-entry. | The current exact-set parser fixture accepts only the four required inputs and rejects incomplete, duplicate, extra, unsafe, and self-entry sets while retaining explicit legacy three-entry compatibility. | PASS | Rust `certificate_hash_parser_requires_exact_complete_unique_set`, `legacy_certificate_without_pdf_remains_verifiable` |
