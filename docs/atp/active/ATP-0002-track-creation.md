...
| --- | --- | --- | --- | --- | --- | --- |
| 1 | `REQ-TRK-001` | Create TD-02 in a workspace without TD-03. | One contained track is created with lifecycle `DRAFT` and workflow ID/version. | A temporary track was created as `DRAFT` with workflow `suno-track` `1.2`. | PASS | Rust `track_creation_builds_exact_folders` |
...
| 6a | `REQ-WFL-002` | Answer code-based generation first with `No`, then with `Yes`; attach source code and the resulting WAV or MP3. | Both controls and requirements are absent for `No`; `Yes` requires the source file and generated audio. Both copies are stored under `02_SUNO/` and their paths appear in generated English documentation. Non-WAV/MP3 audio is rejected for the generated-audio role. | Frontend and native tests cover both branches, both missing requirements, valid Python/WAV/MP3 imports, FLAC rejection for the generated-audio role, and both document paths. | PASS | Vitest `shows source-code and generated-audio uploads only for code-based generation`, `requires source-code and generated WAV/MP3 evidence only after an explicit Yes answer`; Rust `code_based_generation_requires_source_code_and_generated_audio_evidence`, `source_code_import_accepts_text_formats_and_rejects_binary_content`, `code_generated_audio_import_accepts_only_wav_or_mp3`, `guided_german_ui_labels_render_as_english_document_values` |
| 6b | `REQ-WFL-002` | Inspect every applicable Source category and rights field and save German presentation labels. | New input uses clickable, mutually exclusive buttons rather than dropdowns; persistence uses stable English values. An unknown old free-text value remains visible for deliberate reclassification. | Markup tests confirm same-name radio inputs with exactly one checked value. Choice normalization converts known German labels to English values and retains unknown legacy text. The Source renderer uses guided single-choice buttons for all three source/right pairs. | PASS | Vitest `renders required single choices as mutually exclusive buttons`, `stores English values while accepting localized labels and retaining unknown legacy data`, `normalizes every guided track value before saving`; frontend `renderStepContent` |
| 6c | `REQ-WFL-002` | Exercise code generation with post-processing `No`, then `Yes` with zero and multiple operations. | `No` needs no operation and generates no operation claim. `Yes` without an operation is missing; multiple operations pass and persist after reopen. | Native and frontend matrices cover all controller combinations; the workspace-reopen integration retained the exact operation array and optional `Other` note. | PASS | Rust `code_audio_post_processing_requirements_follow_both_controlling_answers`, `new_guided_and_free_text_track_values_survive_workspace_reopen_exactly`; Vitest `evaluates code-audio post-processing only on the applicable branch` |
| 6d | `REQ-WFL-002` | Select suggested Suno model/plan values, then save unknown future and historical strings. | Suggestions are usable but neither field is a closed enum; exact strings survive reopen. | Frontend evaluation accepted non-enum future and historical values. Native reopen retained unrestricted custom strings exactly; UI suggestions are covered by the rendered input configuration. | PASS | Vitest `accepts future and historical free-text Suno model and plan values`; Rust `new_guided_and_free_text_track_values_survive_workspace_reopen_exactly`; frontend `renderStepContent` |
...
| 7a | `REQ-WFL-002` | In Human Work, select a lyrics source, enter the used lyrics and Suno style prompt, then choose multiple confirmed editing steps. | Lyrics source is a mutually exclusive button choice. Non-instrumental lyrics and the style prompt are required; confirmed work is stored as the deterministic multi-selection rather than unrestricted prose. | The frontend renders required single-choice radio buttons for lyrics source, requires the style prompt, conditionally requires used lyrics, and round-trips de-duplicated guided choices. Native workflow evaluation requires `human_work.suno_style_prompt`. | PASS | Vitest `renders required single choices as mutually exclusive buttons`, `stores multiple guided choices deterministically`, `lists only applicable missing items`; Rust embedded workflow validation |
...
Expected Rust evidence includes `track_creation_builds_exact_folders`, `code_based_generation_requires_source_code_and_generated_audio_evidence`, `source_code_import_accepts_text_formats_and_rejects_binary_content`, and `code_generated_audio_import_accepts_only_wav_or_mp3`. Expected Vitest evidence includes `conditional fields > hides external-audio details until yes`, `shows own-audio and sample details only when applicable`, `shows source-code and generated-audio uploads only for code-based generation`, `requires source-code and generated WAV/MP3 evidence only after an explicit Yes answer`, and `navigation > ignores delegated backdrop actions for clicks inside a modal`.
