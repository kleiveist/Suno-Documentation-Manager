...
| 3 | `REQ-ART-001` | Inspect the proposed managed name. | It uses an ASCII-uppercase track stem with `_` separators and does not alter the original source name or file. | The managed paths use `MY_TRACK_AI_ORIGINAL.png`, `MY_TRACK_AI_EDITED.png`, `MY_TRACK_HUMAN_EDITED.png`, and `MY_TRACK_FINAL.png`; legacy paths are not migrated. | PASS | Rust `artwork_import_uses_documented_role_naming` |
...
| 7 | `REQ-ART-001` | Model the full TD-01/TD-03 production sequence. | Present stages use `AI_ORIGINAL`, optional `AI_EDITED` where supplied, `HUMAN_EDITED`, and `FINAL` roles in order. New imports use `_HUMAN_EDITED`; legacy `_EDITED` and finalized paths remain unchanged. | Native naming and legacy-role tests cover the new and old names without migration. | PASS | Rust `artwork_import_uses_documented_role_naming`, `legacy_artwork_role_detection_accepts_old_and_new_human_edited_names` |
| 8 | `REQ-ART-001` | Answer `No` to each TD-05 content check and save. | Each note branch ends, but Artwork Disclosure still requires an explicit `YES` or `NO`; deliberate disclosure `NO` accepts a regular final JPG/PNG, and the Audio assessment stays independent. | Native and frontend evaluation keep the disclosure decision visible after three explicit negative answers and accept a deliberate `NO` without requiring a generated derivative. | PASS | Rust `three_negative_artwork_checks_do_not_disable_the_audio_ai_step`; Vitest `requires an explicit artwork disclosure decision even after three negative content checks` |
| 8a | `REQ-ART-001` | Save multiple human process operations for human artwork and multiple human changes for AI-assisted artwork, including custom details. | Human process notes remain editable. AI-assisted artwork is missing until at least one change is selected; arrays and optional custom details persist. | Frontend requirements enforce the AI-assisted selection. Native reopen retained all selected arrays and free-text details exactly. | PASS | Vitest `requires one or more human changes for AI-assisted artwork`; Rust `ai_assisted_artwork_requires_one_or_more_factual_human_changes`, `new_guided_and_free_text_track_values_survive_workspace_reopen_exactly` |
| 9 | `REQ-ART-001` | Answer `Yes` separately for real person, real event, and trademark/logo. | A factual note and configured evidence become applicable; no legal result is generated. | Conditional visibility/evaluation requires each note only for its own `Yes` answer. Persistence and generated-document tests retain the factual answers/notes and reject predefined legal conclusions. Complete packaged button inspection remains open. | PARTIAL | Vitest conditional-field and requirement tests; Rust `new_guided_and_free_text_track_values_survive_workspace_reopen_exactly`, `conditional_post_processing_and_artwork_facts_render_without_legal_inference` |
