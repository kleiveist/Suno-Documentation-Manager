...
| AS-12 | Run a multi-sample intensity plan | The configured dynamic/fixed target is capped at 25 requests / 300 seconds; every actual range is within track bounds, distinct, non-overlapping, deterministically distributed, and recorded with its provider result/archive hash | Native deterministic planner and adapter fixture tests | Planned |
