...
use crate::model::{
    ...
    FactOrigin, Profile, StepState, StepStatus, SunoLyricsContentSource, SunoLyricsContentType,
...
const EXTERNAL_TIMESTAMP_DISCLAIMER: &str = "The application records the external timestamp evidence and its referenced hash. It does not independently determine the timestamp's legal qualification unless explicitly technically verified.";
...
pub fn generate_external_timestamp_addendum_pdf(
    ...
    let rows = vec![
        TableRow::mono("Certificate ID [System value]", snapshot.certificate_id),
        TableRow::plain(
        ...
        TableRow::plain("Note [User-confirmed fact]", documented(snapshot.note)),
        TableRow::mono(
            "Imported at [System value]",
            documented(snapshot.imported_at),
        ),
        TableRow::plain("Provenance [System value]", documented(snapshot.provenance)),
    ];
