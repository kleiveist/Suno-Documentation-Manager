...
use crate::model::{
    BlockingDeviation, DocumentationAnswer, EvidenceItem, EvidenceRole, FactOrigin, Profile,
    StepState, StepStatus, SunoLyricsContentSource, SunoLyricsContentType, TrackAutomation,
    TrackFields, TrackRecord,
};
use crate::workflow;
use printpdf::{
...
fn render_suno_field(layout: &mut PdfLayout, snapshot: &CertificatePdfSnapshot<'_>) {
    ...
    if !fields.lyrics_source.trim().is_empty() || !fields.lyrics_text.trim().is_empty() {
        ...
        layout.write_wrapped(
            &format!("Legacy text: {}", documented(&fields.lyrics_text)),
            LEFT_MM,
            RIGHT_MM - LEFT_MM,
            7.1,
            BuiltinFont::Helvetica,
...
fn render_integrity_anchors(layout: &mut PdfLayout, snapshot: &CertificatePdfSnapshot<'_>) {
    ...
    layout.write_wrapped(
        &format!(
            "Previous revision count [System verification]: {}",
...
mod tests {
    ...
    #[test]
    fn renders_distinct_generation_identifiers_but_omits_unused_time() {
        let mut fixture = Fixture::new(1);
        ...
        assert!(text.contains("User final generation ID"));
